// Dream Team Travel — localStorage store + seed data (IIFE wrapped)
(function () {
  "use strict";

  const KEY_BOOKINGS = "dtt:bookings:v1";
  const KEY_CLOSURES = "dtt:closures:v1";
  const KEY_SEEDED   = "dtt:seeded:v3";
  ...
  function stats() {
    ...
    const weekDeps = list.filter(b => b.date >= today && b.date <= weekTo);

    return {
      todayBookings: todayDeps.length,
      todayPax: todayDeps.reduce((s, b) => s + (b.pax || 0), 0),
      weekBookings: weekDeps.length,
      weekPax: weekDeps.reduce((s, b) => s + (b.pax || 0), 0),
      revenueWeek: weekDeps.reduce((s, b) => s + (b.totalPrice || 0), 0),
  ...
  function notify() { listeners.forEach(fn => { try { fn(); } catch (e) { console.error("[Store] listener:", e); } }); }
  ...
  function seedIfNeeded() {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const pax = rndChoice([1, 1, 2, 2, 2, 3, 4]);
      ...
      list.push({
        ...
        routeId: route.id, routeCity: route.city,
        date: dateISO, time, pax, tripType, totalPrice,
        pickup: rndChoice(SAMPLE_PICKUPS),
        dropoff: route.city + " — hotel",
        fullname: rndChoice(SAMPLE_NAMES),
        phone: rndPhone(), email: "",
        note: rndChoice(SAMPLE_NOTES),
        whatsappOpt: true, source: "seed",
      });
    }
  ...
  // Public API
  window.Store = {
    all, byRef, byPhone, create, cancel, taken,
    isClosed, closeDeparture, openDeparture,
    stats, upcomingDepartures, bookingsForDeparture,
    onChange,
